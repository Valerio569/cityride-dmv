End-to-end pipeline for CityRide ride-hailing analytics: R-based data cleaning & feature engineering, PostgreSQL star schema implementation, and Tableau dashboard built on top of a consolidated analytics view.
...
r/
R scripts for data cleaning, quality checks, feature engineering, and first exploratory analysis (EDA).

sql/
PostgreSQL scripts to create staging tables, build the star schema (fact + dimensions), and create the analytics view used in Tableau.
...
Place Rides_Data.csv and Drivers_Data.csv in your working directory

Run r/cityride_cleaning_eda.R

Confirm that the exported files are created:
...
Create a database (e.g., cityride_dmv_assignment)

Import drivers_df.csv and rides_df.csv into PostgreSQL as staging tables:
